...
// import { 
//   Plus, Edit2, Trash2, RefreshCw, Layout, 
//   Activity, Image as ImageIcon, Layers, 
...
//   const currentItems = heroes.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

//   // --- LOGIC HANDLERS ---

//   // 1. POWER BUTTON: Toggle Status
...
//       toast.success(`Hero is now ${newStatus ? 'Live' : 'Draft'}`);
...
//   // 2. EDIT BUTTON: Load and Open Form
...
//   // 3. DELETE BUTTON: Permanent Removal
//   const handleDelete = async (hero: Hero) => {
//     if (window.confirm(`PERMANENT DELETE: Are you sure you want to remove "${hero.title}"? This cannot be undone.`)) {
//       try {
//         await heroService.delete(hero.id);
//         toast.success('Deleted permanently from database');
//         fetchHeroes();
//       } catch (error) {
...
//         toast.success('New hero created');
//       }
...
//             HERO <span className="text-indigo-600">UNITS</span>
//           </h1>
//           <div className="flex gap-2 mt-3">
//             <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-900 rounded-full shadow-lg">
//               <div className="h-1.5 w-1.5 rounded-full bg-indigo-400 animate-pulse" />
//               <span className="text-[10px] font-black uppercase text-gray-400">Database: {heroes.length}</span>
//             </div>
//           </div>
//         </div>
...
//                 <th className="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Gallery</th>
...
//                   <td className="px-8 py-6 font-black text-gray-300 text-sm">
...
//                       <span className="text-[10px] font-black px-2 py-0.5 rounded bg-indigo-50 text-indigo-600 w-fit">DID: {hero.domainId}</span>
//                       <span className="text-[10px] font-black px-2 py-0.5 rounded bg-amber-50 text-amber-600 w-fit">CID: {hero.courseId}</span>
//                     </div>
//                   </td>
//                   <td className="px-8 py-6">
...
//                       {/* Power Button */}
//                       <button onClick={() => handleToggleStatus(hero)} className="p-2.5 bg-white border border-gray-100 rounded-xl hover:bg-gray-900 hover:text-white transition-all shadow-sm">
//                         <Power size={16} />
//                       </button>
//                       {/* Edit Button */}
//                       <button onClick={() => handleEdit(hero)} className="p-2.5 bg-white border border-gray-100 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
//                         <Edit2 size={16} />
//                       </button>
//                       {/* Delete Button */}
//                       <button onClick={() => handleDelete(hero)} className="p-2.5 bg-white border border-gray-100 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm">
//                         <Trash2 size={16} />
//                       </button>
//                     </div>
//                   </td>
...
//       {/* PAGINATION */}
//       {totalPages > 1 && (
//         <div className="mt-8 flex items-center justify-between bg-gray-900 p-2 pl-8 rounded-full shadow-2xl">
//           <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Page <span className="text-white">{currentPage}</span> of {totalPages}</span>
//           <div className="flex items-center gap-2">
//             <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-3 rounded-full bg-white/5 text-white hover:bg-white/10 disabled:opacity-20 transition-all"><ChevronLeft size={20}/></button>
//             <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-3 rounded-full bg-white/5 text-white hover:bg-white/10 disabled:opacity-20 transition-all"><ChevronRight size={20}/></button>
//           </div>
//         </div>
//       )}

//       {/* MODAL */}
//       <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setSelectedHero(null); }} title={selectedHero ? 'Modify Configuration' : 'Initialize Hero'} size="xl">
